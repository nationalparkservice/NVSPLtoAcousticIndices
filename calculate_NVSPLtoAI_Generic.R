#CALCULATE A SUITE OF ACOUSTIC INDICES for a directory of NVSPL files
# the indices are calculated over a set time duration set by the user (timestep)
# this version calculated the indices only for data that begins at the top of the choosen timestep 
# for example, if your time step is 10 minutes, then each AI calculation will start at 0, 10, 20, 30, 40, 50 for each hour.
# If the full ten minute sample is not available, the timestep is skipped.
# This version is ideal if you want to break results easily into hours. 
...
## -------------------------------------------------------------------------
#INPUT PARAMETERS
params = dlgOpen(title = "choose parameters file: paramsFileAcousticIndex_project_instrum")$res
load(params)
